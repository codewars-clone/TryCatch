...
const getLikes = async (req, res, next) => {
  try{
    const likes = [];
    const response = await db.collection('likesUser').doc(req.params.userId).get();
...
const sendLike = async (req, res, next) => {
  try{
    ...
    db.collection('userLikes').doc(currentUser.id).set({[likedUser.id]: true});
    db.collection('likedUser').doc(likedUser.id).set({[currentUser.id]: true});
